...
From `CURSORRULES/fork`, you can use:

`upload_rules_pack.ps1`

Example:

```powershell
.\upload_rules_pack.ps1 -ForkRepoUrl "https://github.com/warcklian/awesome-cursorrules.git"
```

Notes:
- Detects the pack folder automatically (requires `.cursorrules` inside the folder).
- Creates a new branch, copies the pack to `rules/<pack-name>-cursorrules-prompt-file/`, commits and pushes.
- Prints a compare URL to open PR quickly.
